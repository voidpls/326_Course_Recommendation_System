...
exports.getRecommendations = async (req, res, next) => {
  ...
  try {
    ...
    if (!user)
      return res
        .status(500)
        .json({ success: false, error: 'Could not find courses for user' })
...
function getPrompt (courses, userInterests) {
  const prompt =
    'Given the following courses taken, interests, and course description list, construct a list of 4-6 recommended courses for this student, along with a short reasoning. Make sure to check course prerequisites, if mentioned.\n' +
